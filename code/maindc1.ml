...
(*let hash_t = Hashtbl.create 1
let replace = Hashtbl.replace
let length = Hashtbl.length
let find = Hashtbl.find
*)
let ascii_array = Array.make 255 0
...
(*this is called when something is loaded onto the stack that is not a function*)
let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        (*| 'l' -> printf "operator l reg 0%o is unimplemented\n%!" reg
         email trevor what to do here, whats difference bewteen 
         regoper and oper, what do we do with the popped val
         what do we do with the wildcard*)
        | 'l' -> (*if (length hash_t) > 0
                 then push (find hash_t reg) thestack
                 else printf "ocamldc: is empty\n%!"*)
                 (*take from the stack*)
                 let asc_num = (ord reg)
                 Array.set ascii_array asc_num (pop thestack) 
        (*| 's' -> printf "operator s reg 0%o is unimplemented\n%!" reg*)
        | 's' -> (*replace hash_t reg (pop thestack)*)
                 (*put on the stack*)
                 push thestack (Array.get ascii_array (ord reg))
...
(*gets called when a function is loaded onto the stack *)
